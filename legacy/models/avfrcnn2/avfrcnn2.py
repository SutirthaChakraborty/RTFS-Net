...
from ...layers import normalizations, activations, ConvNorm, FRCNNBlock
...
class Attention_block(nn.Module):
    def __init__(self, ain_chan=128, vin_chan=128):
        ...
        self.psi = nn.Sequential(ConvNorm(ain_chan, ain_chan, 1, 1, groups=ain_chan), nn.Sigmoid())
        self.relu = nn.ReLU()
...
class AudioVisual(nn.Module):
    ...
    def __init__(
        ...
        self.fusion_levels = fusion_levels if fusion_levels is not None else list(range(self.an_repeats))
        ...
        self.post = nn.Sequential(nn.PReLU(), nn.Conv1d(bn_chan, n_src * in_chan, 1, 1), activations.get(mask_act)())
...
class AVFRCNN2(BaseAVEncoderMaskerDecoder):
    def __init__(
        ...
        **fb_kwargs,
        ...
        # encoder
        encoder = nn.Conv1d(
            in_channels=1, out_channels=n_filters, kernel_size=kernel_size, stride=stride, padding=kernel_size // 2, bias=False
        )
