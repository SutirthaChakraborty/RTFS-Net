...
# LastEditTime: 2022-02-17 11:46:48
###
...
from .frcnn_ctc_cc import FRCNNCTCCC as VideoFRCNN
...
class CTCBlock(nn.Module):
    def __init__(self, in_chan=512, norm_type="gLN", act_type="prelu", depth=5, num_idx=1):
        super().__init__()
...
class FRCNNBlockCTC(nn.Module):
    def __init__(
        ...
        self.spp_dw = FRCNNBlock(in_chan=out_chan, out_chan=out_chan, upsampling_depth=upsampling_depth, norm_type=norm_type, act_type=act_type)

        self.ctc = nn.ModuleList([CTCBlock(out_chan, norm_type, act_type, upsampling_depth, i) for i in range(upsampling_depth)])
...
class AudioVisual(nn.Module):
    ...
    def __init__(
        ...
        self.audio_frcnn = FRCNNBlockCTC(bn_chan, hid_chan, upsampling_depth, norm_type, act_type)
...
class AVFRCNN2CTCCC(BaseAVEncoderMaskerDecoder):
    """
