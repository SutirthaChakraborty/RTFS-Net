...
class InjectionMultiSum(nn.Module):
    def __init__(
        ...
        self.local_embedding = Conv2dActNorm(
        ...
        self.global_embedding = Conv2dActNorm(
            in_chan=self.in_chan,
            out_chan=self.hid_chan,
            kernel_size=self.kernel_size,
            groups=self.groups,
        ...
        self.global_gate = Conv2dActNorm(
            in_chan=self.in_chan,
            out_chan=self.hid_chan,
            kernel_size=self.kernel_size,
            groups=self.groups,
...
class TDANetBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        for features in downsampled_outputs:
            global_features = global_features + F.adaptive_avg_pool2d(features, output_size=shape[-2:])
...
class TDANet(nn.Module):
    ...
    def forward(self, x):
        ...
        return x.view(batch_size, -1, T, freq)
