...
# LastEditTime: 2022-02-24 21:07:43
###
...
from .frcnn_top import FRCNNTOP as VideoFRCNN
...
class Topdown(nn.Module):
    ...
    def forward(self, residual, bottomup, topdown):
        x_fuse = None
        for i in range(len(self.fuse_layers)-1, -1, -1):
            wav_length = topdown[i].shape[-1]
            # import pdb; pdb.set_trace()
            y = torch.cat(
                (
                    bottomup
                    if i == len(self.fuse_layers)-1
                    else F.interpolate(x_fuse, size=wav_length, mode="nearest"),

                    F.interpolate(topdown[i+1], size=wav_length, mode="nearest")
                    if i != len(self.fuse_layers)-1
...
class AudioVisual(nn.Module):
    ...
    def __init__(
        ...
        self.audio_frcnn = nn.ModuleList([
            Bottomup(bn_chan, hid_chan, 5, upsampling_depth, norm_type, act_type),
            Topdown(bn_chan, hid_chan, 5, upsampling_depth, norm_type, act_type)
        ])
    ...
    def fuse(self, a, v):
        ...
        a_res, a_final, a = self.audio_frcnn[0](a)
        v_res, v_final, v = self.video_frcnn.get_frcnn_block(0)[0](v)
        if 0 in self.fusion_levels:
            # print("fusion", 0)
            a_final, v_final = self.get_crossmodal_fusion(0)(a_final, v_final)
        a = self.audio_frcnn[1](a_res, a_final, a)
        v = self.video_frcnn.get_frcnn_block(0)[1](v_res, v_final, v)
        ...
        for i in range(1, self.an_repeats):
            ...
            a_res, a_final, a = self.audio_frcnn[0](self.audio_concat(res_a + a))
            frcnn = self.video_frcnn.get_frcnn_block(i)
            concat_block = self.video_frcnn.get_concat_block(i)
            v_res, v_final, v = frcnn[0](concat_block(res_v + v))
            if i in self.fusion_levels:
                # print("fusion", i)
                a_final, v_final = self.get_crossmodal_fusion(i)(a_final, v_final)
            a = self.audio_frcnn[1](a_res, a_final, a)
            v = self.video_frcnn.get_frcnn_block(0)[1](v_res, v_final, v)
...
class AVFRCNN2TOP(BaseAVEncoderMaskerDecoder):
    """
